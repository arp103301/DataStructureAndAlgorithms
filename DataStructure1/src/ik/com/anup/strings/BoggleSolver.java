...
static TrieNode root;
 public static ArrayList<String> boggle_solver(ArrayList<String> dictionary, ArrayList<String> mat) {
        ...
        char[][] board = new char[m][n];
         for (int i=0; i < m; i++){
            for (int j=0; j < n; j++){
                board[i][j] = mat.get(i).charAt(j);
            }
        }

        for(int i=0; i< m; i++){
            for(int j=0; j< n; j++){
                 dfs (board, i, j, root, resList,dictionary);
...
public static void dfs(char[][] board, int i, int j, TrieNode p, ArrayList<String> res,ArrayList<String> dict) {
    char c = board[i][j];
    if (c == '#' || !p.next.containsKey(c)) return;
    p = p.next.get(c);
    if (p.word != null) {
        res.add(p.word);
        String remIndex = p.word;
        p.word=null;
    ...
    for(int[] dir:dirs){
        ...
        dfs(board, x, y, p, res,dict);
    }
    board[i][j] = c;
}

public static TrieNode buildTrie(ArrayList<String> words) {
    ...
    return root;
     ...
     static boolean remove(TrieNode p,String str, int i){
        if(i== str.length()){
                if(p.next.size() ==0){
                    return true;
                }else{
            ...
            if(remove(p.next.get(ch), str, i+1)){
                p.next.remove(ch);
