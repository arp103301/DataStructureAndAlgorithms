...
//check the version2 solution for more clarity :::::::::::::::::MaximumsubArrayOfSizeK2

//https://www.youtube.com/watch?v=xFJXtB5vSmM


// use deque to check peeklast for inserting within the window.. use 2 for loops ..one from 0 to w-1 and another from w to n
...
Imagine that n is very large and a sliding window of a smaller size w is moving through arr from 
left to right. We need to find the maximum in every position of the sliding window.
...
i-th value in the returned array must be the maximum among arr[i], arr[i + 1], ..., arr[i + w - 1].
Constraints:
...
public class MaximumInSlidingWindow {
    ...
    static ArrayList<Integer> max_in_sliding_window(ArrayList<Integer> arr, Integer w) {
        ...
        // from front to rear in Q, i.e.,
        // arr[Q.front[]] to arr[Q.rear()]
        // are sorted in decreasing order
        Deque<Integer> q = new LinkedList<>();
        /* 1. Process first w (or first window) elements of array */
        for (int i = 0; i < w; i++) {
            if (q.isEmpty()) {
                q.offer(arr.get(i));
            } else {
                ...
                // remove them from Q
                while (!q.isEmpty() && q.peekLast() < arr.get(i))
                	 // Remove from rear
                    q.removeLast();
        ...
        // First element in the deque is the greatest of all withing that window, by design.-- only one reamining on the deque
        result.add(q.peekFirst());
    
        //2.  Process rest of the elements, i.e., from arr[w] to arr[n-1]
 ...
 * implementation). ***********************Elements can be added or removed from either end of a deque***********************
 * in constant time, optimal_solution takes advantage of that property
 */
